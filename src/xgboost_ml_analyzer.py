...
class MLPredictionResult:
    ...
    recommendation: str
    model_version: str
    # PHASE 4: Dynamic zone width (ML-determined)
    zone_width: float = 20.0  # ML-calculated optimal zone width (points)
    zone_type: str = "MEDIUM"  # "TIGHT", "MEDIUM", "WIDE"
    zone_confidence: float = 70.0  # Confidence in zone calculation
    zone_reason: str = ""  # Why this zone width
...
class XGBoostMLAnalyzer:
    ...
    def predict(
        ...
        # PHASE 4: Calculate ML-based zone width using ALL features
        zone_width, zone_type, zone_confidence, zone_reason = self._calculate_ml_zone_width(
        ...
        return MLPredictionResult(
            ...
            # PHASE 4: Zone width from ML analysis
    ...
    def _calculate_ml_zone_width(self, features: Dict) -> Tuple[float, str, float, str]:
        """
        Calculate ML-based zone width using ALL features from ALL tabs

        Uses XGBoost-analyzed data to determine optimal strike defense zone width

        Returns:
            (zone_width, zone_type, confidence, reason)
        """
        reasons = []
        base_zone = 20.0  # Default medium zone

        # FACTOR 1: OI Concentration (PRIMARY - from Option Screener)
        oi_concentration = features.get('oi_concentration_atm', 0)  # TODO: Add this feature
        atm_oi_bias = abs(features.get('atm_oi_bias', 0))

        if atm_oi_bias > 50:
            base_zone = 15.0  # Tight zone - strong OI concentration
            reasons.append("High ATM OI concentration")
        elif atm_oi_bias < 20:
            base_zone = 30.0  # Wide zone - dispersed OI
            reasons.append("Wide OI distribution")
        else:
            base_zone = 20.0  # Medium zone
            reasons.append("Moderate OI spread")

        # FACTOR 2: Volatility Regime (from Volatility Tab)
        vix_level = features.get('vix_level', 15)
        compression_score = features.get('compression_score', 0)

        iv_multiplier = 1.0
        if compression_score < -50:  # Compression → tighter zone
            iv_multiplier = 0.7
            reasons.append("IV compressing (tightening)")
        elif compression_score > 50:  # Expansion → wider zone
            iv_multiplier = 1.3
            reasons.append("IV expanding")
        else:
            reasons.append("IV stable")

        # FACTOR 3: Expiry Proximity (from Volatility Tab)
        is_expiry_week = features.get('expiry_week', 0)
        days_to_expiry = 7  # Default assumption

        dte_multiplier = 1.0
        if is_expiry_week == 1:
            dte_multiplier = 0.7  # Strong pinning near expiry
            days_to_expiry = 2
            reasons.append("Expiry week (strong pinning)")
        else:
            reasons.append("Normal expiry distance")

        # FACTOR 4: Delta/Flow Pressure (from CVD Tab)
        delta_imbalance = abs(features.get('delta_imbalance', 0))
        orderflow_strength = abs(features.get('orderflow_strength', 0))

        delta_pressure = "NEUTRAL"
        if delta_imbalance > 0.5 and orderflow_strength > 0.5:
            delta_pressure = "STRONG"
            reasons.append("Strong delta pressure")
        elif delta_imbalance < 0.2:
            delta_pressure = "WEAK"
            reasons.append("Weak delta pressure")

        # FACTOR 5: Regime Strength (from ML Regime Tab)
        regime_confidence = features.get('regime_confidence', 50)
        trend_strength = abs(features.get('trend_strength', 0))

        if trend_strength > 0.7 and regime_confidence > 70:
            # Strong trending → wider zone (price travels more)
            dte_multiplier *= 1.2
            reasons.append("Strong trending regime")
        elif trend_strength < 0.3:
            # Ranging → tighter zone
            dte_multiplier *= 0.9
            reasons.append("Ranging market")

        # FACTOR 6: Participant Analysis (from Participant Tab)
        institutional_confidence = features.get('institutional_confidence', 50)
        smart_money = features.get('smart_money', 0)

        if smart_money == 1 and institutional_confidence > 70:
            # Smart money active → zone holds better
            base_zone *= 0.9
            reasons.append("Smart money active")

        # FINAL CALCULATION
        zone_width = base_zone * iv_multiplier * dte_multiplier
        zone_width = np.clip(zone_width, 10.0, 35.0)
        ...
        return zone_width, zone_type, confidence, reason

    def analyze_support_resistance_strength(self, features: Dict) -> Dict:
        """
        Analyze whether Support/Resistance is BREAKING or BUILDING STRONG
        Uses ALL data from ALL tabs via XGBoost analysis

        Returns:
            Dict with support/resistance strength analysis
        """
        analysis = {
            'support_status': 'NEUTRAL',  # "BREAKING", "BUILDING", "NEUTRAL"
            'support_strength': 50.0,  # 0-100
        ...
        # SUPPORT ANALYSIS
        near_support = (htf_support_dist < 0.3 or vob_support_dist < 0.3)
        if near_support:
            # Check if bouncing (BUILDING) or breaking (BREAKING)
            if price_change_1 > 0 and price_change_5 > 0:
                # Price bouncing from support
                analysis['support_status'] = 'BUILDING'
                analysis['support_strength'] = 75.0
                reasons.append("Support BUILDING (price bouncing)")
            elif price_change_1 < 0 and price_change_5 < -0.5:
                # Price breaking support
                analysis['support_status'] = 'BREAKING'
                analysis['support_strength'] = 30.0
                reasons.append("Support BREAKING (downward momentum)")
            else:
                analysis['support_status'] = 'TESTING'
                analysis['support_strength'] = 50.0
                reasons.append("Support being TESTED")

        # RESISTANCE ANALYSIS
        near_resistance = (htf_resistance_dist < 0.3 or vob_resistance_dist < 0.3)
        if near_resistance:
            if price_change_1 < 0 and price_change_5 < 0:
                # Price rejecting from resistance
                analysis['resistance_status'] = 'BUILDING'
                analysis['resistance_strength'] = 75.0
                reasons.append("Resistance BUILDING (price rejecting)")
            elif price_change_1 > 0 and price_change_5 > 0.5:
                # Price breaking resistance
                analysis['resistance_status'] = 'BREAKING'
                analysis['resistance_strength'] = 30.0
                reasons.append("Resistance BREAKING (upward momentum)")
            else:
                analysis['resistance_status'] = 'TESTING'
                analysis['resistance_strength'] = 50.0
                reasons.append("Resistance being TESTED")

        # FACTOR 2: Volume Confirmation (from VOB)
        ...
        if volume_concentration > 0.6:
            # High volume at level = strong defense
            if analysis['support_status'] == 'BUILDING':
                analysis['support_strength'] += 15
                reasons.append("High volume at support (strong)")
            if analysis['resistance_status'] == 'BUILDING':
                analysis['resistance_strength'] += 15
                reasons.append("High volume at resistance (strong)")
        elif volume_concentration < 0.3:
            # Low volume = weak defense
            if analysis['support_status'] == 'TESTING':
                analysis['support_strength'] -= 15
                reasons.append("Low volume at support (weak)")
            if analysis['resistance_status'] == 'TESTING':
                analysis['resistance_strength'] -= 15
                reasons.append("Low volume at resistance (weak)")

        # FACTOR 3: Delta/Flow (from CVD Tab)
        ...
        if institutional_sweep == 1:
            # Institutional sweep = likely breakout
            if near_resistance:
                analysis['resistance_status'] = 'BREAKING'
                analysis['resistance_strength'] = 25.0
                reasons.append("Institutional sweep (resistance breaking)")
            elif near_support:
                analysis['support_status'] = 'BREAKING'
                analysis['support_strength'] = 25.0
                reasons.append("Institutional sweep (support breaking)")

        # FACTOR 4: OI Buildup (from Option Screener)
        oi_buildup_pattern = features.get('oi_buildup_pattern', 0)

        if oi_buildup_pattern > 0.5:  # Call buildup
            # Bearish for resistance (sellers adding)
            if near_resistance:
                analysis['resistance_strength'] += 10
                reasons.append("Call OI buildup (resistance strengthening)")
        elif oi_buildup_pattern < -0.5:  # Put buildup
            # Bearish for support (sellers adding)
            if near_support:
                analysis['support_strength'] += 10
                reasons.append("Put OI buildup (support strengthening)")

        # FACTOR 5: Regime Strength (from ML Regime)
        trend_strength = features.get('trend_strength', 0)
        regime_confidence = features.get('regime_confidence', 50)

        if abs(trend_strength) > 0.7 and regime_confidence > 70:
            # Strong trend increases breakout probability
            if trend_strength > 0 and near_resistance:
                analysis['resistance_strength'] -= 15
                reasons.append("Strong uptrend (resistance vulnerable)")
            elif trend_strength < 0 and near_support:
                analysis['support_strength'] -= 15
                reasons.append("Strong downtrend (support vulnerable)")

        # FACTOR 6: Participant Confidence (from Participant Tab)
        institutional_confidence = features.get('institutional_confidence', 50)
        ...
        if institutional_confidence > 70 and retail_confidence < 40:
            ...
            reasons.append("Smart money divergence detected")
